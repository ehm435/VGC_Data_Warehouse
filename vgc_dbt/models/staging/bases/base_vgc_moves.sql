with source as (
    select * from {{ source('vgc_raw', 'vgc_moves') }}
),
...
deduplicated as (
    ...
    qualify row_number() over (
        partition by pokemon_name, move_name, date, regulation 
...
renamed as (
    select
        {{ dbt_utils.generate_surrogate_key(['pokemon_name', 'move_name', 'date', 'regulation']) }} as move_id,
        {{ dbt_utils.generate_surrogate_key(['pokemon_name', 'date', 'regulation']) }} as meta_id,

        cast(pokemon_name as varchar) as pokemon_name,
        cast(move_name as varchar) as move_name,
